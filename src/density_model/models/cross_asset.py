...
and last-time-step cross-asset attention. Emits probabilistic Gaussian
forecasts shaped ``(B, ID, H, 2 * D_y)`` with ``mu`` and ``sigma`` concatenated
on the last axis.

Classes
-------
CausalCrossAssetTransformerConfig
    Pydantic config for the model (architecture + runtime-resolved feature
    dimensions and categorical cardinalities).

CausalCrossAssetTransformer
    Registered panel model under the name ``causal_cross_asset_transformer``.
"""
...
__all__ = ["CausalCrossAssetTransformer", "CausalCrossAssetTransformerConfig"]


class CausalCrossAssetTransformerConfig(ModelConfig):
    """
    ...
    continuous_dim : int or None, default=None
        Number of continuous input channels (``None`` disables the stream).
    ...
    sigma_min: float = Field(default=1e-6, gt=0.0)
...
@register("causal_cross_asset_transformer")
class CausalCrossAssetTransformer(BasePanelModel):
    ...
    def forward(self, batch: PanelBatch) -> torch.Tensor:  # type: ignore[override]
        ...
        if not encoded_streams:
            raise ValueError(
                "CausalCrossAssetTransformer requires at least one active input stream."
            )
