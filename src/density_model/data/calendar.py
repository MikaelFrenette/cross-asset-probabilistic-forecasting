...
class MarketCalendar:
    ...
    def next_session_after(self, session_date: date | pd.Timestamp) -> pd.Timestamp:
        ...
        if self._calendar is not None:
            ...
            schedule = self._calendar.schedule(start_date=search_start.date(), end_date=search_end.date())
            if not schedule.empty:
        ...
        next_values = pd.date_range(start=normalized_date + timedelta(days=1), periods=1, freq=self._fallback_offset)
        if len(next_values) != 1:
            raise ValueError(f"Unable to locate a future session after {normalized_date.date().isoformat()}.")
    ...
    def _build_pandas_market_calendar(self):  # type: ignore[no-untyped-def]
        ...
        try:
            ...
            raise ImportError("pandas_market_calendars must be installed to use market calendar normalization.") from error
    ...
    def _activate_us_equity_fallback(self) -> None:
        ...
        if self.calendar_name not in supported_aliases:
            raise ValueError(
                f"pandas_market_calendars does not expose a calendar matching {self.calendar_name!r}, "
                "and no internal fallback exists for this market."
            )
        self._fallback_offset = CustomBusinessDay(calendar=USFederalHolidayCalendar())
        warnings.warn(
            (
                f"Falling back to approximate US business-day calendar logic for {self.calendar_name}. "
                "This is suitable for daily workflows but may not match historical exchange closures exactly."
            ),
            RuntimeWarning,
            stacklevel=2,
        )
