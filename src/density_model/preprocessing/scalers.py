...
Scalers
-------
Concrete scaling artifacts for continuous preprocessing.
"""
...
from density_model.preprocessing.base import BaseScaler, decode_scalar, encode_scalar
...
class StandardScaler(BaseScaler):
    ...
    id_column : str, default="asset_id"
        Identifier column used to fit per-entity scaling statistics.
    feature_columns : tuple of str or None, default=None
        Continuous feature columns fitted by the scaler.
    mean_ : dict or None, default=None
        Nested mapping of identifier to per-feature means.
    scale_ : dict or None, default=None
        Nested mapping of identifier to per-feature standard deviations.
    variance_ : dict or None, default=None
        Nested mapping of identifier to per-feature variances.
    """
    ...
    def _check_is_fitted(self) -> None:
        """
        Validate that the scaler has been fitted.

        Returns
        -------
        None
            This method raises when the scaler is unfitted.
        """

        if self.mean_ is None or self.scale_ is None or self.variance_ is None or self.feature_columns is None:
            raise ValueError("StandardScaler must be fitted before use.")

    def _validate_frame(self, frame: pd.DataFrame) -> None:
        """
        Validate that a panel frame matches the fitted scaler schema.

        Parameters
        ----------
        frame : pandas.DataFrame
            Panel frame to validate.

        Returns
        -------
        None
            This method raises when feature names are incompatible.
        """
    ...
    def _validate_entity_id(self, entity_id: Any) -> None:
        """
        Validate that a transformed panel contains only fitted identifiers.
        ...
        -------
        None
            This method raises when the identifier is unfitted.
        """
    ...
    def _to_frame(self, values: Any) -> pd.DataFrame:
        """
        Convert input values to a panel DataFrame.
    ...
    def _infer_feature_columns(self, frame: pd.DataFrame) -> tuple[str, ...]:
        """
        Infer continuous feature columns from a panel frame.
    ...
    def _serialize_nested_stats(self, stats: dict[Any, dict[str, float]] | None) -> list[dict[str, Any]]:
        """
        Serialize nested identifier-feature statistics.

        Parameters
        ----------
        stats : dict or None
            Nested statistics mapping.

        Returns
        -------
        list of dict
            Serialized statistics payload.
        """
    ...
    @staticmethod
    def _deserialize_nested_stats(payload: list[dict[str, Any]]) -> dict[Any, dict[str, float]]:
        """
        Deserialize nested identifier-feature statistics.
        ...
        for item in payload:
            ...
            deserialized[entity_id] = {str(key): float(value) for key, value in item["stats"].items()}
        return deserialized
